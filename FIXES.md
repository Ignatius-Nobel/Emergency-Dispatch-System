...
| SAFE-1  | [DONE]  | server/dispatch_grid_environment.py, tests/test_env.py | `step()` after terminal state returns a terminal observation instead of raising IndexError |
| FIX-5   | [DONE]  | server/dispatch_grid_environment.py, tests/test_env_v2.py | Outcome `compute_harm` for `medical` blends unit shortfall (ambulance/police/fire per answer key) with ETA; blocks reward-hack via wrong/minimal units |
