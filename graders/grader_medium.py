"""Grader for Task 2: Ambiguous & Multi-Type Dispatch (medium).
...
try:
    ...
    from server.dispatch_grid_environment import compute_reward, MEDIUM_CALLS
except ImportError:
    try:
        from dispatch_grid.models import DispatchGridAction, EmergencyCall
        from dispatch_grid.server.dispatch_grid_environment import compute_reward, MEDIUM_CALLS
    except ImportError:
        from ..models import DispatchGridAction, EmergencyCall
        from .dispatch_grid_environment import compute_reward, MEDIUM_CALLS


class MediumGrader:
    """Grade Task 2: Ambiguous & Multi-Type Dispatch."""

    def __init__(self) -> None:
        self.last_breakdown: dict[str, Any] = {}
        self._calls_by_id = {c.call_id: c for c in MEDIUM_CALLS}
