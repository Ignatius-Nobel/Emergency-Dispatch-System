"""Grader for Task 4: Resource-Strained Emergencies (crisis).
...
try:
    ...
    from server.dispatch_grid_environment import compute_reward, CRISIS_CALLS
except ImportError:
    try:
        from dispatch_grid.models import DispatchGridAction, EmergencyCall
        from dispatch_grid.server.dispatch_grid_environment import compute_reward, CRISIS_CALLS
    except ImportError:
        from ..models import DispatchGridAction, EmergencyCall
        from .dispatch_grid_environment import compute_reward, CRISIS_CALLS


class CrisisGrader:
    """Grade Task 4: Resource-Strained Emergencies."""

    def __init__(self) -> None:
        self.last_breakdown: dict[str, Any] = {}
        self._calls_by_id = {c.call_id: c for c in CRISIS_CALLS}
